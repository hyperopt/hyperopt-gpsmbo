...
def metropolis_hastings_accept(energy_prev, energy_next, s_rng):
    ...
    #ediff = theano.printing.Print('ediff')(ediff) 
...
def simulate_dynamics(initial_pos, initial_vel, stepsize, n_steps, energy_fn):
    ...
    final_vel = final_vel - 0.5 * stepsize * TT.grad(
        energy.sum(), final_pos,
        consider_constant=[final_pos, final_vel])
...
class HMC_sampler(object):
 ...
 # used in geometric avg. 1.0 would be not moving at all
        ...
        :param energy_fn:
            ...
            The len of this vector is the batch size.
        ...
        avg_acceptance_rate = sharedX(target_acceptance_rate,
                                      'avg_acceptance_rate')
        if s_rng is None:
            s_rng = TT.shared_randomstreams.RandomStreams(12345)
    ...
    def draw(self, **kwargs):
       ...
       """
        self.simulate()
        if self.stepsize.get_value() < 1.5 * self.stepsize_min:
            print 'WARN: HMC stepsize %f close to minimum of %f (acceptance %f)' %(
                self.stepsize.get_value(), self.stepsize_min, self.avg_acceptance_rate.get_value())
        if self.stepsize.get_value() > .9 * self.stepsize_max:
            print 'WARN: HMC stepsize %f close to maximum of %f (acceptance %f)' %(
                self.stepsize.get_value(), self.stepsize_max, self.avg_acceptance_rate.get_value())
